#!/usr/bin/env python3
"""
Test script for Inspire RH56E2 Hand Joint Trajectory Controller
...
class HandTrajectoryTest(Node):
    def __init__(self):
        ...
        self.trajectory_publisher = self.create_publisher(
            JointTrajectory,
            '/inspire_rh56e2_hand_joint_trajectory_controller/joint_trajectory',
            10
        )
    ...
    def close_hand(self):
        ...
        # E2 limits: thumb_yaw=1.658, thumb_pitch=0.62, fingers=1.4381
        positions = [1.2, 0.5, 1.4, 1.4, 1.4, 1.4]
        self.send_trajectory(positions)

    def pinch_gesture(self):
        """Pinch gesture - thumb and index finger"""
        # Better thumb positioning for pinch (E2 has wider thumb yaw range)
        positions = [1.5, 0.4, 1.2, 0.0, 0.0, 0.0]
        self.send_trajectory(positions)

    def point_gesture(self):
        """Pointing gesture - index finger extended"""
        # Close all but index finger
        positions = [0.8, 0.3, 0.0, 1.2, 1.2, 1.2]
        self.send_trajectory(positions)
