...
先进行只读审查，不自动修改源码、替换 `install/`、部署 DTB 或发送机械手命令。
...
初次只读评审不执行；需要复现时，仅对已确认独立 domain、mock-only URDF 的模拟系统运行：
